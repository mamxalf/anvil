...
export default function Index({ events, currentMonth }: CalendarIndexProps) {
  ...
  const navigateMonth = (direction: number) => {
    ...
    router.get('/parent/scheduled_classes', {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  return (
                    <div
                      ...
                      className={`h-20 p-1 border rounded-xl cursor-pointer transition-colors
                        ...
                        hover:bg-green-50`}
                    >
                      <div className={`text-sm font-bold ${isToday ? 'text-kodibot-green' : 'text-gray-700'}`}>
                        {date.getDate()}
                      </div>
                      <div className="flex flex-wrap gap-0.5 mt-1">
                        {dayEvents.slice(0, 3).map((event) => (
                          <div key={event.id} className="w-2 h-2 rounded-full bg-kodibot-green" title={event.title} />
                        ))}
                        {dayEvents.length > 3 && (
        ...
        <div className="space-y-6">
          {selectedDate && eventsByDate[selectedDate.toDateString()] ? (
            <Card className="rounded-2xl">
              ...
              <CardContent className="space-y-3">
                {eventsByDate[selectedDate.toDateString()].map(event => (
                  <Link key={event.id} href={`/parent/scheduled_classes/${event.id}`} className="block p-3 rounded-xl border hover:bg-green-50 transition-colors">
            ...
            <Card className="rounded-2xl">
              <CardHeader>
                <CardTitle className="text-lg">{t('calendar.upcoming', { defaultValue: 'Upcoming Classes' })}</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-sm text-gray-500 text-center py-4">{t('calendar.select_date', { defaultValue: 'Select a date to view classes.' })}</p>
              </CardContent>
            </Card>
          )}
        </div>
