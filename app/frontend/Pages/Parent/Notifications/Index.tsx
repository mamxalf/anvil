      ...
      <div className="space-y-4">
        {notifications.length > 0 ? (
          notifications.map((notification) => (
            <Card
              key={notification.id}
              className={`transition-colors rounded-2xl ${notification.read_at ? 'bg-white opacity-80' : 'bg-kodibot-green/5 border-kodibot-green/20'}`}
            >
              <CardContent className="p-4 flex gap-4 items-start">
                ...
                {!notification.read_at && (
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 text-kodibot-green hover:text-kodibot-green hover:bg-kodibot-green/10 rounded-full"
