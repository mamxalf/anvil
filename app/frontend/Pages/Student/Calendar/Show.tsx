...
export default function Show({ scheduledClass, isRegistered }: ScheduledClassShowProps) {
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      {/* Back Link */}
      <Link href="/student/scheduled_classes" className="flex items-center gap-2 text-gray-500 hover:text-kodibot-orange transition-colors font-medium">
          ...
          <div className="pt-6 border-t border-gray-100">
            {scheduledClass.in_progress && (
              <div className="mb-4 p-4 bg-kodibot-green/10 rounded-xl flex items-center gap-3">
                ...
                <span className="font-bold text-kodibot-green">{t('calendar.class_in_progress', { defaultValue: 'Class is in progress!' })}</span>
              </div>
            )}
            ...
            {isRegistered ? (
              <div className="space-y-4">
                <div className="p-4 bg-kodibot-green/10 rounded-xl text-center">
                  <p className="font-bold text-kodibot-green">✓ {t('calendar.registered', { defaultValue: "You're registered for this class!" })}</p>
                </div>
                {scheduledClass.meeting_url && (
                  <Button asChild className="w-full bg-kodibot-orange hover:bg-kodibot-orange/90 rounded-xl font-bold text-lg py-6">
              ...
              <div className="p-4 bg-gray-100 rounded-xl text-center">
                <p className="text-gray-500">{t('calendar.registration_closed', { defaultValue: 'Registration is closed for this class.' })}</p>
              </div>
            )}
          </div>
        </CardContent>
