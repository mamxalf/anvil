      ...
      <div className="space-y-4">
        {notifications.length > 0 ? (
          notifications.map((notification) => (
            <Card key={notification.id} className={`transition-colors rounded-2xl ${notification.read_at ? 'bg-white opacity-80' : 'bg-kodibot-orange/5 border-kodibot-orange/20'}`}>
              <CardContent className="p-4 flex gap-4 items-start">
                ...
                {!notification.read_at && (
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-kodibot-orange hover:text-kodibot-orange hover:bg-kodibot-orange/10 rounded-full" onClick={() => handleMarkAsRead(notification.id)} title="Mark as read">
