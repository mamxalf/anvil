        ...
        <div className="lg:col-span-2 space-y-8">
          <div>
            <div className="flex gap-2 mb-4">
              <span className="px-3 py-1 bg-kodibot-orange/10 text-kodibot-orange rounded-full text-sm font-bold uppercase tracking-wider">{course.subject}</span>
          ...
          <div className="space-y-6">
            <h3 className="text-2xl font-bold flex items-center gap-3">
              <span className="w-8 h-8 rounded-full bg-kodibot-orange text-white flex items-center justify-center text-sm">📚</span>
              {t('courses.curriculum', { defaultValue: 'Course Curriculum' })}
            </h3>
            <div className="space-y-4">
            {modules && modules.length > 0 ? modules.map((mod: any, index: number) => (
               <Card key={mod.id} className="border-l-4 border-l-kodibot-orange overflow-hidden hover:shadow-md transition-shadow rounded-2xl">
        ...
        <div className="sticky top-8 space-y-6">
           <Card className="overflow-hidden shadow-lg border-none rounded-2xl">
             ...
             <CardContent className="p-8 space-y-6">
                ...
                {isEnrolled ? (
                  <Button className="w-full text-lg py-6 font-bold bg-kodibot-green hover:bg-kodibot-green/90 rounded-xl" asChild>
                     <Link href={`/student/courses/${course.id}/learn`}>{t('courses.continue_learning', { defaultValue: 'Continue Learning' })}</Link>
                  </Button>
                ) : (
                  <Button 
