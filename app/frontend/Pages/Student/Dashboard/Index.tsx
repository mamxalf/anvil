          ...
          <section>
            ...
            {courses && courses.length > 0 ? (
              ...
              <div className="bg-white/60 backdrop-blur-sm rounded-[2.5rem] p-12 text-center border-4 border-dashed border-gray-200 hover:border-kodibot-orange/30 transition-colors group">
                ...
                <Button asChild size="lg" className="bg-gradient-to-r from-orange-500 via-kodibot-orange to-yellow-500 text-white rounded-2xl px-8 shadow-lg shadow-orange-200 hover:shadow-orange-300 hover:scale-105 transition-all text-lg font-bold">
          ...
          <div className="bg-gradient-to-b from-[#00A86B] to-emerald-600 rounded-[2rem] p-6 text-white shadow-xl shadow-emerald-100/50 overflow-hidden relative group flex flex-col">
            ...
            <div className="relative z-10 flex flex-col h-full">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-xl font-black leading-tight">{t('dashboard.student.recent_badges')}</h3>
                </div>
              </div>
              ...
              <div className="flex-grow">
                {recentBadges && recentBadges.length > 0 ? (
                  <div className="grid grid-cols-3 gap-3">
                    {recentBadges.map((badge, i) => (
                      <div key={i} className="flex flex-col items-center text-center gap-2 group/badge">
                        ...
                        <span className="text-[10px] font-bold text-white/90 line-clamp-2 leading-tight">{badge.name}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="w-full h-full min-h-[180px] bg-white/10 backdrop-blur-md rounded-3xl border border-emerald-400/30 flex flex-col items-center justify-center text-center p-6 shadow-inner">
                    <p className="text-white font-bold text-lg mb-1">{t('gamification.no_badges', { defaultValue: 'Belum ada lencana' })}</p>
                    <p className="text-emerald-100 text-sm font-medium">{t('gamification.complete_courses', { defaultValue: 'Ayo selesaikan kursus!' })}</p>
                  </div>
                )}
              </div>
          ...
          <div className="bg-gradient-to-b from-emerald-600 to-emerald-400 rounded-[2rem] p-6 shadow-xl shadow-green-100/50 overflow-hidden relative text-white">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-xl font-black leading-tight">{t('dashboard.student.daily_challenge')}</h3>
                  <p className="text-emerald-100 text-xs font-medium mt-0.5">{t('gamification.complete_and_earn', { defaultValue: 'Selesaikan & dapatkan XP!' })}</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4 bg-white/10 backdrop-blur-md p-4 rounded-[1.5rem] border border-white/20 shadow-sm transition-all">
                  ...
                  <div className="flex-grow opacity-60">
                    <span className="text-base font-bold text-white block line-through decoration-2 decoration-white/50">{t('gamification.login_today', { defaultValue: 'Login hari ini' })}</span>
                    <span className="text-sm font-extrabold text-yellow-300 flex items-center gap-1">
                      +10 XP <span className="text-emerald-100 text-xs font-bold uppercase tracking-wider ml-1 px-1.5 py-0.5 rounded bg-emerald-800/30">{t('common.done', { defaultValue: 'Selesai' })}</span>
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 bg-transparent p-4 rounded-[1.5rem] border border-white/30 hover:bg-white/5 hover:border-white/50 transition-all cursor-pointer group">
                  ...
                  <div className="flex-grow">
                    <span className="text-base font-bold block group-hover:translate-x-1 transition-transform">{t('gamification.complete_quiz', { defaultValue: 'Selesaikan 1 Kuis' })}</span>
                    <span className="text-sm font-extrabold text-yellow-300 shadow-black drop-shadow-sm">+50 XP</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 bg-transparent p-4 rounded-[1.5rem] border border-white/30 hover:bg-white/5 hover:border-white/50 transition-all cursor-pointer group">
                  <div className="w-10 h-10 rounded-full border-[3px] border-white/40 group-hover:border-white transition-colors shrink-0 shadow-sm"></div>
                  <div className="flex-grow">
                    <span className="text-base font-bold block group-hover:translate-x-1 transition-transform">{t('gamification.watch_video', { defaultValue: 'Tonton 1 Video' })}</span>
                    <span className="text-sm font-extrabold text-yellow-300 shadow-black drop-shadow-sm">+20 XP</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8">
                <div className="flex justify-between text-sm font-bold mb-2 items-end">
                  <span className="text-emerald-100 uppercase tracking-wider text-xs">Progress Harian</span>
                  <span className="text-xl font-black">1<span className="text-white/50 text-base">/3</span></span>
                </div>
