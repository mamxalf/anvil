...
export default function AchievementsIndex({ achievements, earnedIds, stats }: AchievementsIndexProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 space-y-8">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
         ...
         <Progress value={progressPercentage} className="h-4 rounded-full bg-gray-100" indicatorClassName="bg-gradient-to-r from-primary to-yellow-500" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {achievements.map((achievement) => {
          ...
          return (
            <Card key={achievement.id} className={`overflow-hidden transition-all duration-300 ${isEarned ? 'border-primary/50 shadow-md transform hover:-translate-y-1' : 'opacity-70 border-gray-200 bg-gray-50'}`}>
               <CardContent className="p-0 relative">
                 {/* Status Badge */}
                 <div className={`absolute top-3 right-3 px-2 py-1 rounded-full text-xs font-bold flex items-center gap-1 ${isEarned ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}>
                    {isEarned ? (
                 ...
                 <div className="p-6 text-center">
                    <div className={`w-20 h-20 mx-auto rounded-full flex items-center justify-center mb-4 text-4xl shadow-inner ${isEarned ? 'bg-gradient-to-br from-yellow-300 to-orange-400 text-white' : 'bg-gray-200 grayscale'}`}>
