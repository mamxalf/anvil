...
export class MazeEngine {
    ...
    private getDirectionSx(direction: DirectionType): number {
        switch (direction) {
            case DirectionType.NORTH: return 0;
            case DirectionType.EAST: return SPRITE_WIDTH;
            case DirectionType.SOUTH: return 2 * SPRITE_WIDTH;
            case DirectionType.WEST: return 3 * SPRITE_WIDTH;
    ...
    private async animateTurn(direction: 1 | -1): Promise<void> {
        return new Promise<void>((resolve) => {
            ...
            this.state.animationState = direction === 1
                ? AnimationStateType.TURN_RIGHT
                : AnimationStateType.TURN_LEFT;
