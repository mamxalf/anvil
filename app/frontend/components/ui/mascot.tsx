...
export default function Mascot({ mood = 'happy', message, className = '', size = 'md' }: MascotProps) {
  ...
  const getMascotImage = () => {
    switch (mood) {
      case 'happy': return '3D-I.png'
      case 'thinking': return '3D-I.png' // Reuse until specific asset available
      case 'celebrating': return '3D-III.png'
      case 'teaching': return '3D-II.png'
      default: return '3D-I.png'
    }
  }
  ...
  return (
    <div className={`flex items-end gap-4 ${className}`}>
      <div className={`mascot-avatar ${sizes[size]} filter drop-shadow-lg transition-transform hover:scale-110 cursor-pointer`}>
        {/* {getMascotEmoji()} */}
        <img 
