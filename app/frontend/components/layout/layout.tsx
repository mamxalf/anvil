  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
            ...
            <div className="hidden md:flex items-center gap-4">
              {user ? (
                <>
                  <Link
                    href="/dashboard"
                    className="text-white/90 hover:text-white font-bold px-4 py-2 hover:bg-white/10 rounded-xl transition-all"
                  >
                    Dashboard
                  </Link>
                  <Link
                  ...
                  <div className="ml-4 flex items-center gap-3 bg-white/10 p-2 pl-4 pr-2 rounded-full border border-white/20">
                    ...
                    <div className="h-8 w-8 rounded-full overflow-hidden border-2 border-white bg-white">
                      <img
                        src={user.avatar_url || '/assets/profile-kodibot.png'}
                        alt={user.name}
                  ...
                  {user.role === 'admin' && (
                    <Link
                      href="/avo"
                      className="text-yellow-200 hover:text-yellow-100 text-sm font-bold"
                    >
                      Admin Panel
                    </Link>
                  )}
                ...
                <>
                  <Link
                    href="/users/sign_in"
                    className="text-white hover:text-yellow-200 font-bold px-4"
                  >
                    Masuk
                  </Link>
                  <Link
      ...
      {(flash.success || flash.notice) && (
        <div
          className="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mx-auto container mt-4 rounded-r-xl shadow-sm"
          role="alert"
        >
      ...
      {(flash.error || flash.alert) && (
        <div
          className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mx-auto container mt-4 rounded-r-xl shadow-sm"
          role="alert"
        >
